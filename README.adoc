...
** Verwenden Sie hierfür den *Einfüge-Sort*

IMPORTANT: Die Verwendung der Sort-Funktion von Collections ist nicht erlaubt.

'''

* Bsp 002: Erstellen Sie eine Klasse Person, sowie eine Klasse `PersonenSortierer`
+
ifdef::env-github[]
image::sortieren-cld-person.svg[]
endif::[]

ifndef::env-github[]
[plantuml,sortieren-cld-person,png]
----
include::plantuml/sortieren-cld-person.puml[]
----
endif::[]

** Lesen Sie die Namen der Datei link:files/names.csv[names.csv] in ein Person-Array ein.
** Verwenden Sie als natürliche Sortierung Nachnamen und als zweites Kriterium den Vornamen
** Generieren Sie für jede Person ein Datum per Zufallsgenerator zwischen 1.1.2006 und 31.12.2008.
** Generieren Sie für jede Person eine Größe per Zufallsgenerator zwischen 160 cm und 200 cm.
** Mit einem Menü kann man auswählen wie die Liste sortiert wird
+
----
1 ... Person aus Datei einlesen
2 ... Liste nach Nachnamen / Vornamen sortiert ausgeben (Grundsortierung)
3 ... Liste nach Alter absteigend ausgeben (Bubble-Sort)
4 ... Liste nach Größe aufsteigend ausgeben (Einfüge-Sort)
0 ... Programm beenden
----

** Zum Sortieren erstellen Sie jeweils ein neues Array, in dem die Personen ausgehend von der ursprünglichen Sortierung eingefügt werden.

IMPORTANT: Die Verwendung der Sort-Funktion von Collections ist nicht erlaubt.

'''

* Bsp 003: Ändern sie Bsp 002 so ab, dass die Sortierung innerhalb eines Arrays erfolgt.
...
* Bsp 004: Sortierung anhand des Body-Mass-Index (BMI)

** Erstellen Sie eine Klasse `Person` mit den Attributen `name`, `size` und `weight`.

** Erstellen Sie eine Methode `bmiRange` zur Klassifizierung des BMI. Verwenden Sie hierzu die Angaben der https://www.who.int/data/gho/data/themes/topics/topic-details/GHO/body-mass-index[WHO^]

*** BMI weniger als 16: "starkes Untergewicht" (severe underweight)
*** BMI zwischen 16 and 17: "mäßiges Untergewicht" (moderate underweight)
*** BMI zwischen 17 and 18.5: "leichtes Untergewicht" (light underweight)
*** BMI zwischen 18.5 and 25: "Normalgewicht" (normal weight)
*** BMI zwischen 25 and 30: "Präadipositas" (pre-obesity)
*** BMI zwischen 30 and 35: "Adipositas Grad I" (obesity grade I)
*** BMI zwischen 35 and 40: "Adipositas Grad II" (obesity grade II)
*** BMI größer als 40: "Adipositas Grad III" (obesity grade III)

** Erstellen Sie mit der Library https://github.com/DiUS/java-faker[JavaFaker^] 10 Personen mit zufälligen Namen.
** Mittels Zufallsgenerator erhalten diese Personen eine Größe (zwischen 160 und 210 cm) und Gewicht (zwischen 60 und 130 kg).

** Speichern Sie diese Personen in einem Array und sortieren Sie dieses Array nach dem BMI mittels Bubble-Sorts in einer selbst erstellten Methode
+
`public static Person[] bubbleSort(Person[] persons, Comparator<Person> comparator)`
+
Dabei ist dieser Methode neben dem zu sortierenden Array auch das Sortierkriterium in Form eines Parameters zu übergeben.

** Verwenden Sie die beiliegenden Klassen im Package `at.htl.lego.student.sort`, sowie die dazugehörigen Unittests.

** Die beiliegenden Unit-Tests sind Teil der Angabe.
...
* Bsp 005: Sortierung anhand des Body-Mass-Index (BMI) auf- oder absteigend

** Ändern Sie das Bsp004 so ab, dass die Sortierung auf- oder absteigend erfolgt.

** Dazu erhält die Methode `bubbleSort` einen weiteren Parameter `boolean ascending`, der angibt, ob die Sortierung auf- oder absteigend erfolgen soll.

** Die Tests wurden natürlich für diesen Fall erweitert.
...
* Bsp 001: Binäre Suche - Testprogramm (von den Folien)

** Ein Ganzzahlenfeld (int[]) wird mit *Zufallszahlen befüllt* und *sortiert*.
** Eine Zahl (hier 3) wird *gesucht*.
** Falls die Zahl nicht gefunden wird, wird das Zahlenfeld neu *befüllt* und die Zahl (3) wird wieder *gesucht*.
** Das wird so lange wiederholt bis die Zahl gefunden wird.

** Bildschirmausgabe
+
----
[5, 6, 8, 22, 23, 25, 49, 63, 70, 86] -> Der Wert 3 ist nicht enthalten.
[4, 9, 30, 38, 45, 48, 50, 66, 84, 85] -> Der Wert 3 ist nicht enthalten.
[1, 5, 22, 32, 46, 49, 70, 77, 82, 97] -> Der Wert 3 ist nicht enthalten.
[3, 37, 44, 52, 52, 56, 84, 90, 91, 91] -> Der Wert 3 ist nicht enthalten.
[5, 26, 39, 42, 62, 62, 64, 68, 78, 89] -> Der Wert 3 ist nicht enthalten.
[7, 16, 22, 29, 31, 37, 39, 57, 63, 91] -> Der Wert 3 ist nicht enthalten.
[8, 8, 14, 26, 31, 31, 36, 65, 91, 99] -> Der Wert 3 ist nicht enthalten.
[14, 31, 38, 53, 58, 73, 79, 94, 94, 98] -> Der Wert 3 ist nicht enthalten.
[7, 14, 26, 31, 43, 49, 52, 70, 78, 93] -> Der Wert 3 ist nicht enthalten.
[1, 3, 17, 65, 68, 76, 85, 88, 92, 97] -> Der Wert 3 ist enthalten.
----

'''

* Bsp 002: Binäre Suche mit Comparator

** Verwenden Sie die Klasse `Person` aus Bsp 004 und suchen Sie aus dem Array von Personen nach einer oder mehreren Personen mit einem bestimmten Kriterium:
*** Name
*** Größe (size)
*** Gewicht (weight)

** Erstellen Sie eine Methode `binarySearch` in der Klasse `PersonMgr`, die eine Liste von Personen, ein Suchkriterium sowie einen Comparator entgegennimmt.
+
Durch die Verwendung geeigneter Comparator-Implementierungen kann nach verschiedenen Kriterien gesucht werden.

* Erstellen Sie eigene Testfälle, um Ihr Programm zu testen
